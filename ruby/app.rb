...
class Isucon5f::WebApp < Sinatra::Base
  use Rack::Session::Cookie, key: "rack_session", secret: (ENV['ISUCON5_SESSION_SECRET'] || 'tonymoris')
  ...
  helpers do
    ...
    def redis
      ...
      @redis = Redis.new(:host => "203.104.208.210")
    end
...
SQL
  ...
  def fetch_api(service, method, uri, headers, params, conf)
    key = case service
      when "ken" then "#{service}_#{conf['keys'][0]}"
      when "ken2" then "#{service}_#{params["zipcode"]}"
      when "surname" then "#{service}_#{params["q"]}"
      when "givenname" then "#{service}_#{params["q"]}"
      else nil
    end

    if key && res = redis.get(key)
      JSON.parse(res)
    else
  ...
  get '/data' do
    ...
    endpoints = {
      "ken"  => ["http://api.five-final.isucon.net:8080/%s", nil, nil],
      "ken2" => ["http://api.five-final.isucon.net:8080/", nil, nil],
      "surname" => ["http://api.five-final.isucon.net:8081/surname", nil, nil],
      "givenname" => ["http://api.five-final.isucon.net:8081/givenname", nil, nil],
      "tenki" => ["http://api.five-final.isucon.net:8988/", "param", "zipcode"],
      "perfectsec" => ["https://api.five-final.isucon.net:8443/tokens", "header", "X-PERFECT-SECURITY-TOKEN"],
      "perfectsec_attacked" => ["https://api.five-final.isucon.net:8443/attacked_list", "header", "X-PERFECT-SECURITY-TOKEN"],
    }

    arg.each_pair do |service, conf|
      cache_enable = false if service=='tenki' || service=='perfectsec' || service=='perfectsec_attacked'
      method, uri_template, token_type, token_key = ["GET"] + endpoints[service]
      headers = {}
      ...
      get_data = {"service" => service, "data" => fetch_api(service, method, uri, headers, params, conf)}
      data << get_data
    end

    cache_control :no_cache unless cache_enable
    json data
  end
  ...
  get '/initialize' do
    ...
    system("rm -rf /var/cache/nginx/cache")
    system("mkdir -p /var/cache/nginx/cache")
  end
end
